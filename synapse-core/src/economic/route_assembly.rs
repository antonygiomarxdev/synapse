...
/// 2. Build the route greedily: for each expert, pick the node that
...
pub fn assemble_route(
    ...
    let mut by_expert: BTreeMap<ExpertId, Vec<(TokensPerMillion, Option<&NodeId>)>> =
        BTreeMap::new();
    ...
    let mut node_counts: BTreeMap<&NodeId, u32> = BTreeMap::new();
    ...
    for expert_id in expert_ids.iter().take(active_per_token as usize) {
        ...
        let best = listings
            ...
            .ok_or_else(|| DomainError::InvalidRoute { reason: "expert has no listings".into() })?;
